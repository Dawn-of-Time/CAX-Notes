...
function syncIngest() {
  ...
  const entries = fs.readdirSync(bufferDir).filter(f => !f.startsWith('.'));
  ...
  function processMdFile(mdPath, imageFiles, baseDir) {
    ...
    if (!data.title) {
      data.title = path.basename(mdPath, '.md');
    }
    ...
    imageFiles.forEach(imgName => {
      const sourceImgPath = path.join(baseDir, imgName);
  ...
  entries.forEach(entry => {
    const entryPath = path.join(bufferDir, entry);
    const stats = fs.statSync(entryPath);

    if (stats.isDirectory()) {
      const noteFolderPath = entryPath;
      ...
      mdFiles.forEach(mdFile => {
        processMdFile(path.join(noteFolderPath, mdFile), imageFiles, noteFolderPath);
      });

      fs.rmSync(noteFolderPath, { recursive: true, force: true });
    } else if (entry.endsWith('.md')) {
      processMdFile(entryPath, [], bufferDir);
